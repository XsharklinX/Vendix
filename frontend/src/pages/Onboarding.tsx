    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-100">¡Bienvenido a Vendix!</h1>
          <p className="text-gray-500 dark:text-slate-400 text-sm mt-1">Configura tu negocio en 3 pasos rápidos</p>
        </div>
          ...
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-all ${
                ...
                'bg-gray-200 dark:bg-slate-600 text-gray-400 dark:text-slate-500'
              }`}>
                {s < step ? <CheckCircle2 size={16} /> : s}
              </div>
              {s < 3 && <div className={`w-12 h-0.5 ${s < step ? 'bg-green-400' : 'bg-gray-200 dark:bg-slate-600'}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="card p-6">
          ...
          {step === 1 && (
            <>
              <h2 className="text-lg font-semibold text-gray-900 dark:text-slate-100 mb-1">¿Qué tipo de negocio tienes?</h2>
              <p className="text-gray-500 dark:text-slate-400 text-sm mb-5">Elige una plantilla y configuramos impuestos, categorías y alertas de stock por ti.</p>
                  ...
                  <button
                    ...
                    className={`flex items-start gap-3 p-3 rounded-xl border-2 text-left transition-all ${
                      template === t.id
                        ...
                        : 'border-gray-200 dark:border-slate-600 hover:border-gray-300 dark:hover:border-slate-600'
                    }`}
                  >
                    <span className="text-2xl">{t.icon}</span>
                    <span>
                      <span className="font-medium text-gray-800 dark:text-slate-200 text-sm block">{t.label}</span>
                      <span className="text-gray-500 dark:text-slate-400 text-xs">{t.description}</span>
                    </span>
                  </button>
                ))}
              </div>
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 mt-4">
                  {BUSINESS_TYPES.map(bt => (
                    ...
                    >
                      ...
                      <span className="font-medium text-gray-800 dark:text-slate-200 text-sm">{bt.label}</span>
                    </button>
                  ))}
                </div>
              )}
          ...
          {step === 2 && (
            <>
              <h2 className="text-lg font-semibold text-gray-900 dark:text-slate-100 mb-1">¿Cuál es tu moneda?</h2>
              <p className="text-gray-500 dark:text-slate-400 text-sm mb-5">La usaremos para mostrar los precios.</p>
              <div className="space-y-2">
                {CURRENCIES.map(c => (
                  ...
                  >
                    <span className="font-medium text-gray-800 dark:text-slate-200 text-sm">{c.label}</span>
                    <span className="text-gray-500 dark:text-slate-400 font-mono text-sm">{c.symbol} ({c.value})</span>
                  </button>
                ))}
              </div>
          ...
          {step === 3 && (
            <>
              <h2 className="text-lg font-semibold text-gray-900 dark:text-slate-100 mb-1">Agrega tu primer producto</h2>
              <p className="text-gray-500 dark:text-slate-400 text-sm mb-5">Opcional — puedes saltarte esto y hacerlo después.</p>

              {error && (
                <div className="mb-4 p-3 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-lg text-red-700 dark:text-red-300 text-sm">{error}</div>
              )}
              ...
              <button
                className="w-full text-center text-sm text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:hover:text-slate-300 mt-3"
                onClick={handleFinish}
                disabled={loading}
              >
                Saltar por ahora
