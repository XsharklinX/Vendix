  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="p-6 space-y-4">
        {totalDebt > 0 && (
          <div className="card p-4 bg-orange-50 border-orange-100 flex items-center gap-3">
            <div className="w-10 h-10 bg-orange-100 rounded-xl flex items-center justify-center">
              <Truck size={18} className="text-orange-600" />
            </div>
            <div>
              <p className="text-sm font-bold text-orange-800">Total que les debes</p>
              <p className="text-xl font-bold text-orange-600">{formatCurrency(totalDebt, cur)}</p>
            </div>
          </div>
        )}
                    ...
                    <tr key={s.id} className="table-row">
                      ...
                      <td className="table-cell">
                        <div className="space-y-0.5">
                          {s.phone && <div className="flex items-center gap-1.5 text-sm text-gray-600"><Phone size={12} className="text-gray-400" />{s.phone}</div>}
                          {s.email && <div className="flex items-center gap-1.5 text-xs text-gray-400"><Mail size={11} />{s.email}</div>}
                          {!s.phone && !s.email && <span className="text-gray-400 text-sm">—</span>}
                        </div>
                      </td>
                      <td className="table-cell text-gray-500">{s.document || '—'}</td>
                      <td className="table-cell text-right">
                        <span className={`text-base font-bold ${s.pendingDebt > 0 ? 'text-red-600' : 'text-green-600'}`}>
                          {formatCurrency(s.pendingDebt, cur)}
                        </span>
                      </td>
                      <td className="table-cell">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => openEdit(s)} className="btn-ghost text-xs px-2.5 py-1.5 text-blue-600 hover:bg-blue-50">
                            <Edit2 size={14} /> Editar
                          </button>
                          <button onClick={() => handleDelete(s)} className="btn-ghost text-xs px-2.5 py-1.5 text-red-500 hover:bg-red-50">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </td>
